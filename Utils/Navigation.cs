...
public static class Navigation
{
    ...
    /// <summary>
    /// Given a starting <see cref="Point2D_rs"/> and a directional symbol of '^' (north), 'v' (south), '&gt;' (east),
    /// or '&lt;' (west) returns a new <see cref="Point2D_rs"/> representing the next location.
    /// </summary>
    public static Point2D_rs NextLocation(this Point2D_rs origin, char direction) => direction switch
    {
        '^' => new Point2D_rs(x: origin.X, y: origin.Y + 1),
        'v' => new Point2D_rs(x: origin.X, y: origin.Y - 1),
        '>' => new Point2D_rs(x: origin.X + 1, y: origin.Y),
        '<' => new Point2D_rs(x: origin.X - 1, y: origin.Y),
        _ => throw new ArgumentException("Unknown direction symbol")
    };
}